...
const embeddedPatientData: PatientData = {
  patients: [
    {
...
// Load patient data from embedded data
export async function loadPatientData(): Promise<PatientData> {
  try {
    // Return embedded data instead of fetching from external file
    return embeddedPatientData;
  } catch (error) {
